...
module Stream (takeS, takeWhileS, maybeToStream, isMature, fmap, pure, (<*>), empty, (<|>), (>>=), mzero, mplus, Stream (..), Delayable, immature) where
...
class MonadPlus nondet => Delayable nondet where

    immature :: nondet x -> nondet x


instance Delayable Stream where
