...
vmapMVal :: (LogicType a, Monad m) => (forall x. (LogicType x) => Var x var -> m (Var x var')) -> WithLogic a var -> m (WithLogic a var')
vmapMVal f x = let (con, elems) = quote x in construct con <$> mapM (\(Field x') -> Field <$> f `vmapM` x') elems

vmapMVal_ :: (LogicType a, Monad m) => (forall x. (LogicType x) => Var x var -> m ()) -> WithLogic a var -> m ()
vmapMVal_ f x = let (_, elems) = quote x in mapM_ (\(Field x') -> f `vmapM_` x') elems

vmap :: (LogicType a) => (forall x. (LogicType x) => Var x var -> Var x var') -> Logic a var -> Logic a var'
vmap f x = runIdentity $ vmapM (Identity . f) x

showLogic :: (Show (WithLogic a var), Show (Var a var)) => Logic a var -> String
showLogic (Free v) = show v
