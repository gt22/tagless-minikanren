...
defaultUnif :: (Alternative rel, LogicVar a) => (forall t. LogicVar t => L' t var -> L' t var -> rel ()) -> a var -> a var -> rel ()
...
field :: (LogicVar t) => L' t var -> Field a var
...
quote0 :: String -> (forall var. t var) -> (Constructor t, [Field t v])
quote0 c r = (con0 c r, [])

quote1 :: (LogicVar a) => String -> (forall var. L' a var -> t var) -> L' a v -> (Constructor t, [Field t v])
quote1 c r x = (con1 c r, [field x])

quote2 :: (LogicVar a, LogicVar b) => String -> (forall var. L' a var -> L' b var -> t var) -> L' a v -> L' b v -> (Constructor t, [Field t v])
quote2 c r x y = (con2 c r, [field x, field y])

quote3 :: (LogicVar a, LogicVar b, LogicVar c) => String -> (forall var. L' a var -> L' b var -> L' c var -> t var) -> L' a v -> L' b v -> L' c v -> (Constructor t, [Field t v])
