...
evalFromRead readVar (Free v) = do
    x <- readVar v
    pure $ fromMaybe (error $ "Unbound variable: " ++ show v) x
...
(===) :: (LogicType a, Kanren rel) => L a rel -> WithLogic a (KVar rel) -> rel ()
a === b = unify a (Ground b)

(<=>) :: (LogicType a, Kanren rel) => L a rel -> L a rel -> rel ()
a <=> b = unify a b

(<->) :: (LogicType a, Kanren rel) => L a rel -> a -> rel ()
a <-> b = unify a (Ground $ project b)
...
occursCheck x (Ground y) = let (_, ys) = quote y in any (\(Field y') -> occursCheck x y') ys