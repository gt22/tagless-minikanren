...
module V2.Interpreters.SubstKanren where

import Control.Monad
import Control.Applicative
import V2.MiniKanren
import MiniKanren (EqVar(varEq), Var, Logic, Relation(Relation))
...
instance (Delayable nondet) => MiniKanren (KEval s nondet) (KEval s nondet) (KEval s nondet) (KEval s nondet) (SVar s) where
    ...
    argVar x = (makeVar (Just x) >>=)

    sucunif = trivial
    nonunif = failing
    failing = lift $ OptionT Nothing
    trivial = return ()

    conj = (>>)
    disj = (<|>)

instance (Delayable nondet) => MiniKanrenEval (KEval s nondet) (KEval s nondet) (KEval s nondet) (KEval s nondet) (SVar s) where
